...
def convert_input_range(acquisition_mode, input_range, board_kind, justcheck=False):
    ...
    if input_range in input_range_dic[acquisition_mode].keys():
        ...
        if acquisition_mode == "pm":    
            ...
            raise ValueError("Possible values (0p qnd 0m modes, in mV) :40, 80, 100, 160, 200, 250, 400, 500, 800, 1000, 1600, 2000, 2500, 4000, 5000, 8000, 1e4, 1.6e4, 2e4, 3.2e4, 8e4")
...
def convert_window_function(window_function_name, samplesPerWindow):
    ...
    if window_function_name == "floattop":
...
class ATSBoard(BaseInstrument):
    ...
    def _current_config(self, dev_obj=None, options={}):
        ...
        base = self._conf_helper('acquisition_length_sec', 'samples_per_record', 'sample_rate', 'clock_type', 'acquisition_mode', 'input_range',
                                 'trigger_channel_1', 'trigger_channel_2', 'trigger_level_1',
                                 'trigger_level_2', 'bw_limited_a', 'bw_limited_b', 'trigger_delay', 'active_channels', 'trigger_mode')
        return extra+base
    ...
    def _trigger_level_2_setdev(self, val):
        ...
        self._ats_trigger_level_2.set(convert_trigger_level(val))
        self._boardConfigured = False
        print("Run ConfigureBoard to take modification into account")
    ...
    def get_data(self, data, active_channels):
        """
        ...
        """
        returned_data = [data["t"]]
        for channel in active_channels:
            returned_data.append(data[channel])
    ...
    def get_screen_size(self):
        ...
        If mode is "pm", the size of the screen is twice the input range
        Otherwise mode is "0m" or "0p" and size of the screen is the input range

        Output:
            - screen_size (int): Size of the recording window
        """
        # If "pm" mode, screen_size = 2 * input_range
        if self.acquisition_mode.get()=="pm": 
            return 2 * self.input_range.get()
        
        # Otherwise screen_size = input_range        
    ...
    def _readval_getdev(self):
        ...
        # Initialize data
        data = {"t":np.linspace(0, self.acquisition_length_sec.get(), samplesPerRecord),
                "A":np.array([]),
                "B":np.array([])}
        ...
        for i in range(buffer_count):
            self.buffers.append(ats.DMABuffer(self._board.handle, self.board_info["C_sample_type"], bytesPerBuffer))
        ...
        screen_size = self.get_screen_size()  # Size of the screen
        ...
        try:
            ...
            while buffersCompleted < buffersPerRecord:
                ...
                for i in range(channel_count):
                    data[channels[i]] = np.concatenate((data[channels[i]], (received_data[i*samplesPerBuffer:(i+1)*samplesPerBuffer]-zero_value)/zero_value*screen_size), axis=0)
                buffersCompleted += 1
        ...
        return self.get_data(data, self.active_channels.get())
    ...
    def _readval_all_getdev(self):
        ...
        channelCount = len(self.active_channels.get())

        data = {"t":np.linspace(0, self.acquisition_length_sec.get(), postTriggerSamples), 
                "A":[],
                "B":[]}
        
        # Compute the number of bytes per record and per buffer
        memorySize_samples = self.board_info["MemorySize"]
        bytesPerSample = self.board_info["BytesPerSample"]
        samplesPerRecord = preTriggerSamples + postTriggerSamples
        bytesPerRecord = bytesPerSample * samplesPerRecord
        bytesPerBuffer = bytesPerRecord * recordsPerBuffer * channelCount
        # TODO: Select number of DMA buffers to allocate
        bufferCount = self.buffer_count.get()

        # Allocate DMA buffers

        sample_type = self.board_info["Sample_type"]
        c_sample_type = self.board_info["C_sample_type"]

        buffers = []
        for i in range(bufferCount):
            buffers.append(ats.DMABuffer(self._board.handle, c_sample_type, bytesPerBuffer))
            ...
            while buffersCompleted < buffersPerAcquisition:
                ...
                for i in range(recordsPerBuffer):
                    for j in range(channelCount):
                        data[channels[j]].append((received_data[(2*j+i)*samplesPerRecord:(2*j+i+1)*samplesPerRecord]-zero_value)/zero_value*screen_size)
    ...
    def _fetch_getdev(self):
        ...
        return self.get_data(self.data, channels)  # Return data in a shape that can be registered in a file 
    ...
    def make_average(self, signals):
        ...
        """
        nbRecords = len(signals)-1
        return [signals[0], sum(signals[1:]) / nbRecords]
    ...
    def _average_getdev(self):
        ...
        return self.make_average(signals)
    ...
    def make_fft(self, data):
        ...
        yf = rfft(data[1], n=nfft)  # FFT data
    ...
    def _rabi_getdev(self):
        ...
        for i in range(1, n+1):
            ...
            thresholds = self.detection_threshold(self.smooth_curve([data[0], data[i]],
                                                                     nbpoints_sliding_mean),
                                                  trigger_level_descend,
                                                  trigger_level_ascend)
            detected_threshold.append(thresholds[0])
            detected_threshold.append(thresholds[1])
    ...
    def _create_devs(self):
        ...
        self._ext_input_range = MemoryDevice(400, autoinit=True, doc=
            "In memory device. Do not modify.")
        self._ats_input_range = MemoryDevice(convert_input_range("pm", 400, self.board_info["Kind"]), autoinit=True, doc=
            "In memory device. Do not modify.")
        self._devwrap("input_range", setget=True, autoinit=True)

        self._devwrap("acquisition_mode", setget=True, autoinit=True)

        self._ext_trigger_channel_1 = MemoryDevice("A", autoinit=True, doc=
            "In memory device. Do not modify.")
        self._ext_trigger_channel_2 = MemoryDevice("", autoinit=True, doc=
            "In memory device. Do not modify.")
        self._ats_trigger_channel_1 = MemoryDevice(convert_trigger_channel("A"), autoinit=True, doc=
            "In memory device. Do not modify.")
